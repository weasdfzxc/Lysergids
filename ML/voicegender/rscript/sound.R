packages <- c('tuneR', 'seewave','pbapply','fftw')
...
# Load data
datas <- processFolder('./data/wav')
#females <- processFolder('data')

# Set labels.
datas$label <- 1
#females$label <- 2
data <- rbind(datas)
data$label <- factor(data$label, labels=c('pending'))
...
write.csv(data, file='voice.csv', row.names=F)
